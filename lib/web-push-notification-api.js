...
export async function handleWebPushNotificationApi({ request, env, path, viewerRole }) {
  try {
    if (viewerRole !== ADMIN_ROLE) throw new WebPushApiError("Admin session is required", 403, "ADMIN_REQUIRED");
    ...
    if (path.length === 2 && request.method === "GET") {
      return webPushJson(await readWebPushStatus(env));
    }
    if (path.length === 3 && path[2] === "subscription" && request.method === "POST") {
      return webPushJson(await registerWebPushSubscription(request, env), 201);
    }
    if (path.length === 3 && path[2] === "subscription" && request.method === "DELETE") {
      return webPushJson(await revokeWebPushSubscription(request, env));
    }
    if (path.length === 3 && path[2] === "test" && request.method === "POST") {
      return webPushJson(await queueWebPushTest(env), 202);
    }
...
export async function readWebPushStatus(env) {
  ...
  const [device, latestTest, dispatcherRaw] = await Promise.all([
    env.DB.prepare(
      `SELECT id AS deviceId, generation, device_name AS deviceName,
       ...
       WHERE status = 'active' AND transport = 'webpush'
...
async function registerWebPushSubscription(request, env) {
  ...
  const existing = await env.DB.prepare(
    `SELECT id AS deviceId, generation
     FROM call_note_devices
     WHERE status = 'active' AND transport = 'webpush' AND target_fingerprint = ?
     LIMIT 1`
  ...
  if (existing) {
    ...
    await audit(env, "notification.webpush.refresh", existing.deviceId, {
      deviceName: metadata.deviceName,
      platform: metadata.platform
    });
    return readWebPushStatus(env);
  }
  ...
  await env.DB.batch([
    ...
    env.DB.prepare(
      `INSERT INTO call_note_devices (
        ...
        last_registered_at, last_seen_at, revoked_at, revoke_reason, updated_at, transport
       ) VALUES (
      ...
      nowIso, metadata.deviceName, pairCodeId, nowIso, nowIso, nowIso, nowIso, nowIso
    ),
    env.DB.prepare(
      `UPDATE call_note_push_deliveries
       SET send_state = 'retry_wait', next_attempt_at = ?, last_error_code = '', updated_at = ?
       WHERE kind <> 'connection_test' AND send_state IN ('waiting_target', 'blocked_config')
         AND accepted_at = ''`
    ).bind(nowIso, nowIso),
    env.DB.prepare(
      `INSERT INTO audit_logs
        (id, actor, action, entity_type, entity_id, before_json, after_json)
       VALUES (?, 'admin', 'notification.webpush.register', 'notification_device', ?, '', ?)`
    ).bind(auditId, deviceId, JSON.stringify({
      deviceName: metadata.deviceName,
      platform: metadata.platform,
      generation,
...
async function revokeWebPushSubscription(request, env) {
  const secret = requireNotificationSecret(env);
  const body = await readBoundedJson(request);
  const subscription = normalizeSubscription(body.subscription);
  const canonical = JSON.stringify(subscription);
  ...
  if (!device) return { removed: false, ...(await readWebPushStatus(env)) };
  ...
  await env.DB.batch([
    ...
    env.DB.prepare(
      `INSERT INTO audit_logs
        (id, actor, action, entity_type, entity_id, before_json, after_json)
       VALUES (?, 'admin', 'notification.webpush.revoke', 'notification_device', ?, '', ?)`
    ).bind(crypto.randomUUID(), device.deviceId, JSON.stringify({ revokedAt: nowIso }))
  ]);

  return { removed: true, ...(await readWebPushStatus(env)) };
}

async function queueWebPushTest(env) {
  const status = await readWebPushStatus(env);
  ...
  await env.DB.batch([
    env.DB.prepare(
      ...
      status.device.generation, nowIso, nowIso, nowIso, nowIso
    ),
    env.DB.prepare(
      `INSERT INTO audit_logs
        (id, actor, action, entity_type, entity_id, before_json, after_json)
       VALUES (?, 'admin', 'notification.webpush.test', 'push_delivery', ?, '', ?)`
    ).bind(crypto.randomUUID(), notificationId, JSON.stringify({ queuedAt: nowIso }))
  ]);
...
async function audit(env, action, entityId, after) {
  await env.DB.prepare(
    `INSERT INTO audit_logs
      (id, actor, action, entity_type, entity_id, before_json, after_json)
     VALUES (?, 'admin', ?, 'notification_device', ?, '', ?)`
