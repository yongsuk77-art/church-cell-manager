    ...
    env.DB.prepare(
      `SELECT id AS deviceId, generation, device_name AS deviceName,
       ...
       WHERE status = 'active' AND transport = 'webpush'
         AND user_id = ?${deviceChurchScoped ? " AND church_id = ?" : ""}
       ORDER BY generation DESC
       LIMIT 1`
    ).bind(
      viewer.id, ...(deviceChurchScoped ? [viewer.churchId || "church-seosan"] : [])
    ).first(),
    env.DB.prepare(
      `SELECT notification_id AS notificationId, send_state AS sendState,
       ...
       WHERE kind = 'connection_test' AND target_user_id = ?${
         deliveryChurchScoped ? " AND church_id = ?" : ""
       }
       ORDER BY created_at DESC
       LIMIT 1`
    ).bind(
      viewer.id, ...(deliveryChurchScoped ? [viewer.churchId || "church-seosan"] : [])
    ).first(),
...
async function registerWebPushSubscription(request, env, viewer) {
  ...
  const deviceChurchScoped = await d1HasColumn(env, "call_note_devices", "church_id");
  const deliveryChurchScoped = await d1HasColumn(env, "call_note_push_deliveries", "church_id");
  const auditChurchScoped = await d1HasColumn(env, "audit_logs", "church_id");
  ...
  const existing = await env.DB.prepare(
    ...
    fingerprint, viewer.id,
    ...(deviceChurchScoped ? [viewer.churchId || "church-seosan"] : [])
  ).first();
  if (existing) {
  ...
  await env.DB.batch([
    ...
    env.DB.prepare(
      ...
      nowIso, nowIso, viewer.id,
      ...(deliveryChurchScoped ? [viewer.churchId || "church-seosan"] : [])
    ),
    env.DB.prepare(
      `INSERT INTO call_note_devices (
        ...
        transport, user_id${deviceChurchScoped ? ", church_id" : ""}
       ) VALUES (
        ?, 'active', ?, ?, ?, ?, ?, 1, 1, ?, 1, ?, 'pwa-web-push/1', 'granted',
        1, ?, ?, '', ?, ?, ?, '', '', ?, 'webpush', ?${deviceChurchScoped ? ", ?" : ""}
       )`
    ).bind(
      deviceId, generation, credentialHmac, TARGET_KIND, ciphertext, fingerprint,
      nowIso, metadata.deviceName, pairCodeId, nowIso, nowIso, nowIso,
      nowIso, nowIso, viewer.id,
      ...(deviceChurchScoped ? [viewer.churchId || "church-seosan"] : [])
    ),
    env.DB.prepare(
    ...
    }), ...(auditChurchScoped ? [viewer.churchId || "church-seosan"] : []))
  ]);
...
async function revokeWebPushSubscription(request, env, viewer) {
  ...
  await env.DB.batch([
    ...
    env.DB.prepare(
      ...
      crypto.randomUUID(), viewerAuditActor(viewer), device.deviceId,
...
async function queueWebPushTest(env, viewer = ownerViewer()) {
  ...
  await env.DB.batch([
      ...
      `INSERT INTO call_note_push_deliveries
        (notification_id, dedupe_key, kind, reminder_id, note_id, visit_id,
         ...
         next_attempt_at, created_at, updated_at, target_user_id${
           deliveryChurchScoped ? ", church_id" : ""
         })
       VALUES (?, ?, 'connection_test', '', '', '', ?, ?, ?, 'pending', 0, ?, ?, ?, ?${
         deliveryChurchScoped ? ", ?" : ""
       })`
    ).bind(
    ...
    env.DB.prepare(
      ...
      crypto.randomUUID(), viewerAuditActor(viewer), notificationId,
      JSON.stringify({ queuedAt: nowIso }),
      ...(auditChurchScoped ? [viewer.churchId || "church-seosan"] : [])
    )
  ]);
...
async function audit(env, actor, action, entityId, after, churchId = "church-seosan") {
  const churchScoped = await d1HasColumn(env, "audit_logs", "church_id");
  await env.DB.prepare(
    `INSERT INTO audit_logs
      (id, actor, action, entity_type, entity_id, before_json, after_json${
        churchScoped ? ", church_id" : ""
      })
     VALUES (?, ?, ?, 'notification_device', ?, '', ?${churchScoped ? ", ?" : ""})`
  ).bind(
    crypto.randomUUID(), actor, action, entityId,
    JSON.stringify(after || {}), ...(churchScoped ? [churchId || "church-seosan"] : [])
  ).run();
}
