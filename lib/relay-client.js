...
const CREDENTIAL_VERIFY_TIMEOUT_MS = 5 * 1000;
...
export async function resolveRelayClientConfiguration(env) {
  if (typeof env?.DB?.prepare !== "function") return inspectRelayClientConfiguration(env);
  let row;
  try {
    row = await env.DB.prepare(
      `SELECT site_id AS siteId, site_origin AS siteOrigin,
        relay_base_url AS baseUrl, key_id AS keyId,
        secret_ciphertext AS secretCiphertext, status
       FROM relay_client_credentials WHERE singleton = 1`
    ).first();
  } catch (error) {
    if (isMissingCredentialTable(error)) return inspectRelayClientConfiguration(env);
    return invalidStoredConfiguration("RELAY_CREDENTIAL_STORE_UNAVAILABLE");
  }
  if (!row) return inspectRelayClientConfiguration(env);
  if (row.status !== "active") return invalidStoredConfiguration("RELAY_SITE_NOT_REGISTERED");

  let baseUrl;
  let siteOrigin;
  try {
    baseUrl = canonicalRelayBaseUrl(row.baseUrl);
    siteOrigin = canonicalSiteOrigin(row.siteOrigin);
  } catch {
    return invalidStoredConfiguration("RELAY_STORED_CREDENTIAL_INVALID");
  }
  const siteId = String(row.siteId || "").toLowerCase();
  const keyId = String(row.keyId || "");
  if (!UUID_PATTERN.test(siteId) || !RELAY_KEY_ID_PATTERN.test(keyId)) {
    return invalidStoredConfiguration("RELAY_STORED_CREDENTIAL_INVALID");
  }
  let secret;
  try {
    secret = await decryptRelayClientSecret(
    ...
    return invalidStoredConfiguration("RELAY_CREDENTIAL_DECRYPT_FAILED");
  }
  if ([...secret].length < 32) {
    return invalidStoredConfiguration("RELAY_STORED_CREDENTIAL_INVALID");
  }
...
export async function sendRelayDelivery({ env, siteId, delivery, device }) {
  ...
  const type = String(delivery.kind || "");
  const noteId = type === "memo_reminder"
    ? String(delivery.noteId || "").toLowerCase()
    : "";
  if (type === "memo_reminder") assertUuid(noteId, "noteId");
  return relayRequest(env, siteId, "POST", "/v1/deliveries", {
    ...
    type,
    reminderId: type === "memo_reminder" || type === "visit_alarm" || type === "today_pastoral"
...
export async function verifyRelayClientCredentials({
  env,
  siteId,
  baseUrl,
  keyId,
  ...
  const configuration = inspectRelayClientConfiguration({
    RELAY_BASE_URL: baseUrl,
    RELAY_KEY_ID: keyId,
    RELAY_HMAC_SECRET: secret
  });
  if (!configuration.ready) {
    throw new RelayClientError("Push relay credentials are invalid", configuration.errorCode, { status: 400 });
  }
  const normalizedSiteId = String(siteId || "").toLowerCase();
  assertUuid(normalizedSiteId, "siteId");
  const payload = await relayRequestWithConfiguration(
    env,
    configuration,
    normalizedSiteId,
    "POST",
    "/v1/site-verifications",
    null,
    CREDENTIAL_VERIFY_TIMEOUT_MS
  );
  if (payload?.code !== "RELAY_CREDENTIALS_VALID"
    || payload?.siteId !== normalizedSiteId
    || payload?.keyId !== keyId) {
    throw new RelayClientError(
      "Push relay credential verification response was invalid",
...
async function relayRequest(env, siteIdValue, method, path, body, timeoutMs) {
  ...
  if (configuration.siteId && configuration.siteId !== siteId) {
    throw new RelayClientError("Stored relay site does not match", "RELAY_SITE_ID_MISMATCH", { status: 503 });
  }
  return relayRequestWithConfiguration(env, configuration, siteId, method, path, body, timeoutMs);
}

async function relayRequestWithConfiguration(env, configuration, siteId, method, path, body, timeoutMs) {
  ...
  try {
    ...
    response = typeof env?.RELAY?.fetch === "function"
  ...
  if (response.status >= 300 && response.status < 400) {
    throw new RelayClientError(
      "Push relay redirects are not allowed",
...
export function canonicalRelayBaseUrl(value) {
...
function invalidStoredConfiguration(errorCode) {
  return {
    ready: false,
    errorCode,
    baseUrl: "",
...
function isMissingCredentialTable(error) {
  return /no such table:\s*relay_client_credentials|no such table.*relay_client_credentials/i
