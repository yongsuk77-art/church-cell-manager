...
async function disconnectOwnDevice(request, env, deviceId) {
  ...
  return notificationJson({ deviceId, disconnected: true, serverTime: nowIso });
}
...
async function disconnectAdminDevice(request, env, deviceId) {
  ...
  const nowIso = new Date().toISOString();
  if (device.status !== "revoked") await revokeDevice(env, deviceId, "admin_disconnect", nowIso);
...
async function queueTestNotification(request, env, deviceId) {
  ...
  if (!schedulerReady || !dispatcher.fcmConfigured
    || !dispatcher.notificationSecretConfigured || !dispatcher.senderEnabled || !dispatcherHealthy) {
  ...
  await env.DB.batch([
    ...
    env.DB.prepare(
      `UPDATE call_note_push_deliveries
       ...
       WHERE device_id = ? AND kind = 'memo_reminder' AND send_state NOT IN ('accepted', 'cancelled', 'dead')`
    ).bind(nowIso, nowIso, deviceId)
  ]);
}
...
function publicDelivery(row) {
  return {
    ...
    deviceId: String(row.deviceId || ""),
    kind: row.kind === "connection_test" ? "connection_test" : "memo_reminder",
