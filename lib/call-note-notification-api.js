...
import {
  RelayEnrollmentApiError,
  completeRelayEnrollmentRequest,
  createRelayEnrollmentRequest,
  inspectRelayEnrollmentRequest,
  readRelayEnrollmentStatus,
  reconcileRelayEnrollmentConfiguration
} from "./relay-enrollment-api.js";
import {
  SiteIdentityError,
...
const DELIVERY_KINDS = new Set(["memo_reminder", "visit_alarm", "connection_test", "today_pastoral"]);
...
export async function handleCallNoteNotificationApi({ request, env, path, viewerRole }) {
  try {
    ...
    if (path[2] === "relay-enrollments" && path.length === 5) {
      if (request.method !== "POST") return methodNotAllowed(["POST"]);
      if (path[4] === "inspect") {
        return notificationJson(
          await inspectRelayEnrollmentRequest(request, env, path[3])
        );
      }
      if (path[4] === "complete") {
        return notificationJson(
          await completeRelayEnrollmentRequest(request, env, path[3])
        );
      }
    }
    ...
    if (path[2] === "devices" && path.length === 5 && path[4] === "memo-session") {
      if (request.method !== "POST") return methodNotAllowed(["POST"]);
      return await createMemoSession(request, env, path[3]);
    }
    ...
    if (path[2] === "notifications" && path.length === 4) {
      if (request.method !== "GET") return methodNotAllowed(["GET"]);
      return await getNotificationDetail(request, env, path[3]);
    }
    ...
    if (error instanceof RelayEnrollmentApiError) {
      return notificationJson(
        { error: error.message, code: error.code || "RELAY_ENROLLMENT_FAILED" },
        Number(error.status || 500)
      );
    }
    if (error instanceof MobileMemoAuthError) {
      return notificationJson(errorPayload(error), error.status);
    }
...
async function handleAdminRequest(request, env, path) {
  ...
  if (path.length === 4 && path[3] === "relay-enrollments") {
    if (request.method !== "POST") return methodNotAllowed(["POST"]);
    return notificationJson(await createRelayEnrollmentRequest(request, env), 201);
  }
...
async function getAdminStatus(request, env) {
  ...
  const relayConfiguration = await reconcileRelayEnrollmentConfiguration(
    env,
    await resolveRelayClientConfiguration(env)
  );
  const relayEnrollment = await readRelayEnrollmentStatus(env, relayConfiguration);
...
async function createMemoSession(request, env, deviceId) {
  ...
  const seen = await env.DB.prepare(
    `UPDATE call_note_devices
     ...
     WHERE id = ? AND generation = ? AND status IN ('active', 'unregistered')`
  ).bind(nowIso, nowIso, device.id, device.generation).run();
  if (Number(seen?.meta?.changes || 0) !== 1) {
...
async function getNotificationDetail(request, env, notificationId) {
  ...
  const row = await env.DB.prepare(
    `SELECT delivery.notification_id AS notificationId, delivery.kind,
      delivery.reminder_id AS reminderId, delivery.scheduled_at AS scheduledAt,
      COALESCE(note.id, '') AS noteSourceId, COALESCE(note.title, '') AS noteTitle,
      COALESCE(note.body, '') AS noteBody,
      COALESCE(visit.id, '') AS visitSourceId, COALESCE(visit.summary, '') AS visitSummary,
      COALESCE(visit.prayer, '') AS visitPrayer, COALESCE(visit.action, '') AS visitAction,
      COALESCE(member.name, '') AS memberName
     FROM call_note_push_deliveries delivery
     LEFT JOIN notes note
       ON delivery.kind = 'memo_reminder' AND note.id = delivery.note_id
         AND COALESCE(note.deleted_at, '') = ''
     LEFT JOIN visit_notes visit
       ON delivery.kind = 'visit_alarm' AND visit.id = delivery.visit_id
     LEFT JOIN members member
       ON visit.member_id = member.id
     WHERE delivery.notification_id = ?
       AND delivery.device_id = ?
       AND delivery.device_generation = ?`
  ).bind(notificationId, device.id, device.generation).first();
  if (!row) {
    throw new NotificationApiError("Notification is unavailable for this device", 404, "NOTIFICATION_NOT_FOUND");
  }

  const content = notificationDetailContent(row);
  return notificationJson({
    schemaVersion: "2",
    siteId: siteIdentity.siteId,
    siteOrigin: siteIdentity.siteOrigin,
    notificationId: String(row.notificationId || "").toLowerCase(),
    type: row.kind,
    reminderId: String(row.reminderId || "").toLowerCase(),
    noteId: row.kind === "memo_reminder" ? String(row.noteSourceId || "").toLowerCase() : "",
    scheduledAt: normalizeStoredDate(row.scheduledAt),
    route: `reminders/${String(row.notificationId || "").toLowerCase()}`,
    title: content.title,
    body: content.body,
...
async function synchronizeRelayDevice({
  ...
  const configuration = await resolveRelayClientConfiguration(env);
  ...
  await env.DB.batch([
    ...
    env.DB.prepare(
      `UPDATE call_note_push_deliveries
       ...
       WHERE device_id = ? AND kind IN ('memo_reminder', 'visit_alarm', 'today_pastoral')
...
function notificationDetailContent(row) {
  if (row.kind === "memo_reminder") {
    if (!row.noteSourceId) {
      throw new NotificationApiError("Notification content is no longer available", 410, "NOTIFICATION_CONTENT_UNAVAILABLE");
    }
    return {
      title: String(row.noteTitle || ""),
      body: String(row.noteBody || "")
    };
  }
  if (row.kind === "visit_alarm") {
    if (!row.visitSourceId) {
      throw new NotificationApiError("Notification content is no longer available", 410, "NOTIFICATION_CONTENT_UNAVAILABLE");
    }
    const memberName = cleanText(row.memberName, 120);
    const summary = String(row.visitSummary || "").trim();
    const prayer = String(row.visitPrayer || "").trim();
    const action = visitActionNote(row.visitAction);
    const sections = [summary];
    if (prayer && !summary.includes(prayer)) sections.push(`기도제목: ${prayer}`);
    if (action && !sections.join("\n").includes(action)) sections.push(`후속조치: ${action}`);
    return {
      title: memberName ? `${memberName} 심방 알람` : "심방 알람",
      body: sections.filter(Boolean).join("\n")
    };
  }
  if (row.kind === "connection_test") {
    return {
      title: "공동체관리 알림 테스트",
      body: "웹과 심방콜노트 앱의 알림 연결이 정상입니다."
    };
  }
  if (row.kind === "today_pastoral") {
    return {
      title: "오늘의 목양",
      body: "오늘 확인할 목양 항목이 있습니다."
    };
  }
  throw new NotificationApiError("Notification type is unsupported", 410, "NOTIFICATION_TYPE_UNSUPPORTED");
}

function visitActionNote(value) {
  const text = String(value || "").trim();
  if (!text.startsWith("visit-meta:")) return text;
  try {
    const parsed = JSON.parse(text.slice("visit-meta:".length));
...
function pushTransport(env) {
  ...
  if (value === "direct" || value === "relay" || value === "webpush") return value;
