...
async function communityOverview(env, viewer) {
  ...
  if (viewerCanManageUsers(viewer)) {
    const [invites, submissions, managedUsers] = await Promise.all([
      listNewcomerInvites(env),
      listNewcomerSubmissions(env),
      listManagedUsers(env)
    ]);
...
async function handleUsers(request, env, path, viewer) {
  ...
  if (request.method === "GET" && !id) return json({ users: await listManagedUsers(env) });
...
async function listManagedUsers(env) {
  const rows = await env.DB.prepare(
    `SELECT id, username, display_name AS displayName, role,
     ...
     FROM app_users ORDER BY status, role, display_name`
  ).all();
  const cells = await env.DB.prepare(
    "SELECT user_id AS userId, cell_id AS cellId FROM app_user_cells ORDER BY user_id, cell_id"
  ).all();
  const cellsByUser = groupValues(cells.results || [], "userId", "cellId");
  return [
    {
      ...publicViewer(ownerViewer()),
      status: "active",
      lastLoginAt: "",
      createdAt: "",
      updatedAt: ""
    ...
    ...(rows.results || []).map((row) => ({
      ...
      hasGlobalScope: row.role === "pastor",
...
async function listAssignableUsers(env) {
...
async function createUser(request, env, viewer) {
  ...
  const cellIds = await normalizeUserCellIds(env, role, body.cellIds);
  ...
  const statements = [
    env.DB.prepare(
      `INSERT INTO app_users (
        ...
        can_edit, status, created_at, updated_at
       ) VALUES (?, ?, ?, ?, ?, ?, ?, 'active', ?, ?)`
    ).bind(
      user.id, user.username, user.displayName, user.role, passwordHash,
      user.canViewSensitive ? 1 : 0, user.canEdit ? 1 : 0, now, now
    ),
    ...cellIds.map((cellId) => env.DB.prepare(
      "INSERT INTO app_user_cells (user_id, cell_id, created_at) VALUES (?, ?, ?)"
...
async function updateUser(request, env, viewer, id) {
  ...
  const previous = await readManagedUser(env, id);
  ...
  const cellIds = await normalizeUserCellIds(
    env,
    role,
    body.cellIds === undefined ? previous.cellIds : body.cellIds
  );
  ...
  const statements = [
    env.DB.prepare(
      `UPDATE app_users SET display_name = ?, role = ?, password_hash = ?,
        can_view_sensitive = ?, can_edit = ?, status = ?, updated_at = ?
       WHERE id = ?`
    ).bind(
      updated.displayName, updated.role, passwordHash, updated.canViewSensitive ? 1 : 0,
      updated.canEdit ? 1 : 0, updated.status, updatedAt, id
    ),
    env.DB.prepare("DELETE FROM app_user_cells WHERE user_id = ?").bind(id),
    ...cellIds.map((cellId) => env.DB.prepare(
      "INSERT INTO app_user_cells (user_id, cell_id, created_at) VALUES (?, ?, ?)"
  ...
  if (status === "disabled") {
    statements.push(
      env.DB.prepare("DELETE FROM auth_auto_login_tokens WHERE user_id = ?").bind(id),
      env.DB.prepare(
        `UPDATE call_note_devices SET status = 'revoked', revoked_at = ?,
          revoke_reason = 'user_disabled', updated_at = ?
         WHERE user_id = ? AND status IN ('active', 'pending', 'unregistered')`
      ...
      env.DB.prepare(
        `UPDATE call_note_push_deliveries SET send_state = 'cancelled',
         ...
         WHERE target_user_id = ? AND send_state NOT IN ('accepted', 'cancelled', 'dead')`
      ).bind(updatedAt, updatedAt, id)
    );
  }
...
async function readManagedUser(env, id) {
...
async function normalizeUserCellIds(env, role, values) {
  ...
  const rows = await env.DB.prepare("SELECT id FROM cells").all();
...
async function createAssignment(request, env, viewer) {
  ...
  await assertAssigneeAccess(env, assignment.assigneeUserId, member.cellId);
...
async function updateAssignment(request, env, viewer, id) {
  requireViewerEdit(viewer);
  const previous = await readAssignment(env, id);
  ...
  const statements = [env.DB.prepare(
    `UPDATE pastoral_assignments SET title = ?, assignee_user_id = ?, status = ?,
      due_date = ?, note = ?, completed_at = ?, updated_at = ? WHERE id = ?`
  ).bind(
    assignment.title, assignment.assigneeUserId, assignment.status, assignment.dueDate,
    assignment.note, assignment.completedAt, assignment.updatedAt, id
  )];
  ...
  if (["completed", "cancelled"].includes(assignment.status)) {
    ...
    ).bind(assignment.updatedAt, assignment.updatedAt, id));
  }
  statements.push(auditStatement(
    env,
    viewer,
...
function assignmentInsertStatement(env, assignment) {
  return env.DB.prepare(
    `INSERT INTO pastoral_assignments (
      ...
      due_date, note, completed_at, created_by_user_id, created_at, updated_at
     ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).bind(
...
async function readAssignment(env, id) {
  return env.DB.prepare(
    `SELECT id, member_id AS memberId, source_kind AS sourceKind, source_key AS sourceKey,
     ...
     FROM pastoral_assignments WHERE id = ?`
  ).bind(id).first();
}

async function assertAssigneeAccess(env, userId, cellId) {
  const assignee = await readViewerById(env, userId);
...
async function handleNewcomers(request, env, path, viewer) {
  ...
  if (section === "invites") {
    if (request.method === "GET" && !id) return json({ invites: await listNewcomerInvites(env) });
  ...
  if (section === "submissions") {
    if (request.method === "GET" && !id) return json({ submissions: await listNewcomerSubmissions(env) });
    if (request.method === "PATCH" && id) return reviewNewcomerSubmission(request, env, viewer, id);
  }
  return json({ error: "Method not allowed" }, 405);
}

async function listNewcomerInvites(env) {
  const rows = await env.DB.prepare(
    `SELECT id, label, expires_at AS expiresAt, max_submissions AS maxSubmissions,
     ...
     FROM newcomer_invites ORDER BY active DESC, expires_at DESC, created_at DESC`
  ).all();
...
async function createNewcomerInvite(request, env, viewer) {
  ...
  await env.DB.batch([
    env.DB.prepare(
      ...
      viewer.id, invite.createdAt, invite.updatedAt
    ),
...
async function updateNewcomerInvite(request, env, viewer, id) {
  const previous = await env.DB.prepare(
    `SELECT id, label, expires_at AS expiresAt, max_submissions AS maxSubmissions,
      submission_count AS submissionCount, active, created_at AS createdAt,
      updated_at AS updatedAt FROM newcomer_invites WHERE id = ?`
  ).bind(id).first();
  ...
  await env.DB.batch([
    ...
    ).bind(updated.label, updated.expiresAt, updated.maxSubmissions, updated.active ? 1 : 0, updated.updatedAt, id),
...
async function createNewcomerSubmission(request, env, invite) {
  ...
  const duplicates = await findDuplicateMembers(env, name, phone);
  const now = new Date().toISOString();
  const submission = {
    id: crypto.randomUUID(),
    inviteId: invite.id,
...
async function listNewcomerSubmissions(env) {
  const rows = await env.DB.prepare(
  ...
  const memberRows = await env.DB.prepare("SELECT id, name, phone, cell_id AS cellId FROM members").all();
...
async function reviewNewcomerSubmission(request, env, viewer, id) {
  const submission = await readNewcomerSubmission(env, id);
  ...
  const cell = await env.DB.prepare("SELECT id FROM cells WHERE id = ?").bind(cellId).first();
  ...
  if (useExistingMemberId) {
    const existing = await env.DB.prepare("SELECT id FROM members WHERE id = ?").bind(useExistingMemberId).first();
  ...
  const duplicates = await findDuplicateMembers(env, submission.name, submission.phone);
...
async function readNewcomerSubmission(env, id) {
  const row = await env.DB.prepare(
    `SELECT id, invite_id AS inviteId, name, phone, birth, address,
      family_details AS familyDetails, consent_at AS consentAt, status,
      duplicate_member_ids AS duplicateMemberIds, created_at AS createdAt,
      updated_at AS updatedAt
...
async function findDuplicateMembers(env, name, phone) {
  const rows = await env.DB.prepare(
    `SELECT id, name, phone, cell_id AS cellId, title
  ...
  const [familyRows, memberRows, visitRows, prayerRows, attendanceRows] = await Promise.all([
    env.DB.prepare(
      `SELECT id, name, note, created_by_user_id AS createdByUserId,
        created_at AS createdAt, updated_at AS updatedAt
       FROM families ORDER BY name, updated_at DESC`
    ).all(),
    env.DB.prepare(
      `SELECT link.family_id AS familyId, link.member_id AS memberId,
       ...
       JOIN cells cell ON cell.id = member.cell_id
       WHERE COALESCE(member.trashed_at, '') = ''
    ...
    env.DB.prepare(
      `SELECT records.member_id AS memberId, records.attendance_status AS attendanceStatus,
       ...
       WHERE sessions.id IN (
         SELECT id FROM sunday_attendance_sessions ORDER BY attendance_date DESC LIMIT 1
       )`
    ).all()
  ]);
...
async function createFamily(request, env, viewer) {
  ...
  await env.DB.batch([
    env.DB.prepare(
      `INSERT INTO families (id, name, note, created_by_user_id, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?)`
    ).bind(family.id, family.name, family.note, family.createdByUserId, now, now),
...
async function updateFamily(request, env, viewer, id) {
  ...
  const previous = await env.DB.prepare(
    "SELECT id, name, note, created_at AS createdAt, updated_at AS updatedAt FROM families WHERE id = ?"
  ).bind(id).first();
  ...
  await env.DB.batch([
    env.DB.prepare("UPDATE families SET name = ?, note = ?, updated_at = ? WHERE id = ?")
      .bind(updated.name, updated.note, updated.updatedAt, id),
...
async function replaceFamilyMembers(request, env, viewer, id) {
  requireViewerEdit(viewer);
  const family = await env.DB.prepare("SELECT id, name, updated_at AS updatedAt FROM families WHERE id = ?")
    .bind(id)
    .first();
...
async function deleteFamily(env, viewer, id) {
  requireViewerEdit(viewer);
  await assertFamilyAccess(env, viewer, id);
  const previous = await env.DB.prepare("SELECT id, name, note FROM families WHERE id = ?").bind(id).first();
  if (!previous) throw httpError("가족을 찾을 수 없습니다", 404, "FAMILY_NOT_FOUND");
  await env.DB.batch([
...
async function assertFamilyAccess(env, viewer, familyId) {
  ...
  const visible = (rows.results || []).some((row) => viewerCanAccessCell(viewer, row.cellId));
  if (!visible && (rows.results || []).length) throw httpError("가족 자료에 접근할 권한이 없습니다", 403, "FAMILY_SCOPE_DENIED");
}
...
async function buildCommunityReport(env, viewer, range, period) {
  const [cellsResult, membersResult, visitsResult, attendanceResult, tasksResult, prayersResult, assignmentsResult] = await Promise.all([
    env.DB.prepare("SELECT id, name, sort_order AS sortOrder FROM cells ORDER BY sort_order, name").all(),
    env.DB.prepare(
      `SELECT id, cell_id AS cellId, name, registered_at AS registeredAt
       FROM members WHERE COALESCE(archived_at, '') = '' AND COALESCE(trashed_at, '') = ''`
    ).all(),
    env.DB.prepare(
    ...
    env.DB.prepare(
      `SELECT record.member_id AS memberId, record.attendance_status AS attendanceStatus,
       ...
       WHERE session.attendance_date BETWEEN ? AND ?`
    ).bind(range.startDate, range.endDate).all(),
    env.DB.prepare(
