...
test("admin bootstrap includes the memo workspace role, notes, and categories", async () => {
  ...
  try {
    ...
    const response = await apiRequest(fixture.env, ["bootstrap"], "GET");
    assert.equal(response.status, 200);
    const payload = await response.json();
    assert.equal(payload.viewerRole, "admin");
    assert.equal(payload.notes.length, 1);
    assert.equal(payload.notes[0].id, created.id);
    assert.deepEqual(payload.noteCategories.map((category) => category.id), ["personal", "visitation", "admin"]);
  } finally {
...
function createFixture() {
  ...
  sqlite.exec(`
    ...
    CREATE TABLE members (
      ...
      long_absent INTEGER NOT NULL DEFAULT 0,
    ...
    CREATE TABLE visit_notes (
      ...
      alarm_state TEXT NOT NULL DEFAULT '',
