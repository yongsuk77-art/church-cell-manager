...
const migrationSql = readFileSync(
  new URL("../migrations/0018_multi_tenant_notification_relay.sql", import.meta.url),
  "utf8"
);
const SITE_ID_PATTERN = /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/;

test("0018 preserves existing community and notification data and adds safe relay defaults", () => {
  ...
  try {
    ...
    assert.equal(settings["notification.siteOrigin"], "");
...
test("0018 creates a different canonical site id for each fresh database", () => {
...
test("0018 does not replace a previously fixed site identity", () => {
  const sqlite = createPreMigrationDatabase();
