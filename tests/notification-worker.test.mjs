...
test("disabled relay dispatcher still persists its tenant and transport readiness", async () => {
  ...
  sqlite.exec(`
    ...
    INSERT INTO app_settings (key, value, updated_at) VALUES
      ...
      ('notification.siteOrigin', 'https://church-cell-manager.pages.dev', '2026-07-14T00:00:00.000Z');
  `);
  try {
    ...
    assert.equal(stored.siteOrigin, "https://church-cell-manager.pages.dev");
...
test("scheduled dispatcher repairs an active device that registration could not sync to the relay", async () => {
  ...
  try {
    const first = await synchronizeActiveRelayTarget(
      env,
      { siteId: TEST_SITE_ID, siteOrigin: "https://church-cell-manager.pages.dev" },
      notificationSecret
    );
...
function createTimedDispatcherDatabase() {
  ...
  sqlite.exec(`
    ...
    INSERT INTO app_settings (key, value, updated_at) VALUES
      ...
      ('notification.siteOrigin', 'https://church-cell-manager.pages.dev',
