...
const migration = readFileSync(new URL("../migrations/0020_note_style_and_attachments.sql", import.meta.url), "utf8");
const syncMigration = readFileSync(new URL("../migrations/0021_mobile_memo_sync.sql", import.meta.url), "utf8");
const trashMigration = readFileSync(new URL("../migrations/0022_note_trash_purge.sql", import.meta.url), "utf8");
const attachmentMigration = readFileSync(new URL("../migrations/0023_note_attachment_idempotency.sql", import.meta.url), "utf8");
const categoryMigration = readFileSync(new URL("../migrations/0024_persistent_note_categories.sql", import.meta.url), "utf8");
const editableCategoryMigration = readFileSync(new URL("../migrations/0025_editable_optional_note_categories.sql", import.meta.url), "utf8");
...
test("the main page opens a dedicated memo page from the toolbar and member detail", () => {
  assert.match(appHtml, /id="memoCenterBtn"/);
...
test("the main memo navigation uses a calm single-label button treatment", () => {
  ...
  assert.match(memoButton, /<span>메모함<\/span>/);
  assert.doesNotMatch(memoButton, /새 페이지|<small>/);
  assert.match(memoButton, /M8 8h8M8 12h8M8 16h5/);
  assert.match(appStyles, /\.memo-center-link/);
});

test("the existing visit and attendance toolbar controls remain available", () => {
  ...
  assert.match(visitButton, /심방일자/);
  assert.match(attendanceButton, /주일출석/);
});
...
test("memo member links consistently use church-member wording", () => {
  ...
  assert.match(appHtml, /title="이 성도의 메모" aria-label="이 성도의 메모"/);
});
...
test("call-note review remains available in both the pastoral settings and memo inbox", () => {
  assert.match(appHtml, /id="callNoteInbox"/);
  assert.match(appScript, /loadCallNoteImports/);
  assert.match(memoHtml, /id="callNoteSection"[\s\S]*id="callNoteGrid"/);
});
