...
1. 이 사이트 D1의 `0015`부터 `0025`까지 pending migration 적용
2. 갱신된 Pages 배포
3. `PUSH_SEND_ENABLED=false` 상태로 알림 Worker 배포
4. 중앙 Relay 운영자에게 이 사이트의 `siteId`와 `siteOrigin`을 전달하고 사이트별 키 발급
5. Pages와 알림 Worker에 동일한 site key와 알림 secret을 secret으로 설정
6. 페어링·테스트 알림·ACK 확인 후 알림 Worker의 실제 발송 활성화
7. Android 앱 배포

중앙 Relay의 스키마와 Worker는 중앙 운영자가 별도로 관리합니다. 이 저장소에는 중앙 Relay 구현이나 Firebase 서비스 계정을 넣지 않습니다.
